...
def ApproximateMinimumCircle(points_list: List[Type[Point]]) -> Type[Circle]:
    ...
    (min_x, max_x) = min(points_list, key=lambda x: x.getX()), max(
        points_list, key=lambda x: x.getX())

    (min_y, max_y) = min(points_list, key=lambda x: x.getY()), max(
        points_list, key=lambda x: x.getY())

    possible_pairs = [(min_x, max_x), (min_y, max_y)]
    ...
    for point in points_list:
        if not (minimum_circle.isPointInside(point)):
            ...
            minimum_circle.moveCenter(
                (d - minimum_circle.getRadius()) / 2, point.getVectorToPoint(c))
