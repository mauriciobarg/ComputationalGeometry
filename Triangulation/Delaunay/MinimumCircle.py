...
def MinimumCircle(points_list: List[Type[Point]], other_points: List[Type[Point]] = []) -> Type[Circle]:
    ...
    if (len(points_list) == 0):
        ...
        triangle_vertices = [(points_list[0], points_list[1]),
                             (points_list[0], points_list[2]), (points_list[1], points_list[2])]
        triangle_sides = sorted(
            triangle_vertices, key=lambda x: x[0].distanceToPointSquared(x[1]))
        circle_points = max(triangle_vertices,
                            key=lambda x: x[0].distanceToPointSquared(x[1]))
        ...
        if (points_list[0].isColinear(points_list[1], points_list[2])):
            ...
            side_a = triangle_sides[0][0].distanceToPointSquared(
                triangle_sides[0][1])
            side_b = triangle_sides[1][0].distanceToPointSquared(
                triangle_sides[1][1])
            side_c = triangle_sides[2][0].distanceToPointSquared(
                triangle_sides[2][1])
            ...
            if (side_a > side_b + side_c):
                ...
                ux = (1 / d) * ((ax**2 + ay**2)*(by - cy) +
                                (bx**2 + by**2)*(cy - ay) + (cx**2 + cy**2)*(ay - by))
                uy = (1 / d) * ((ax**2 + ay**2)*(cx - bx) +
                                (bx**2 + by**2)*(ax - cx) + (cx**2 + cy**2)*(bx - ax))
                center = Point(ux, uy)
